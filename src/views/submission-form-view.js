...
function createInputRow({ id, label, type = 'text', placeholder }) {
  ...
  const requiredMark = createElement('span', 'required');
...
export function createSubmissionFormView() {
  const container = createElement('section', 'card');
  const title = createElement('h1');
  title.textContent = 'Validate submission';

  const helper = createElement('p', 'helper');
  helper.textContent = 'Complete required fields and validate your submission.';

  const form = document.createElement('form');
  form.noValidate = true;

  const authorRow = createInputRow({
    id: 'authorNames',
    label: 'Author names',
    placeholder: 'Author One, Author Two',
  });
  const affiliationRow = createInputRow({
    id: 'affiliations',
    label: 'Affiliations',
    placeholder: 'University, Lab',
  });
  const emailRow = createInputRow({
    id: 'contactEmail',
    label: 'Contact email',
    type: 'email',
    placeholder: 'author@example.com',
  });
  const abstractRow = createInputRow({
    id: 'abstract',
    label: 'Abstract',
    type: 'textarea',
  });
  const keywordsRow = createInputRow({
    id: 'keywords',
    label: 'Keywords',
  ...
  const sourceRequired = createElement('span', 'required');
  sourceRequired.textContent = ' *';
  sourceLabel.appendChild(sourceRequired);
  ...
  const fileRequired = createElement('span', 'required');
