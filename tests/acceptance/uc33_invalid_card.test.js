...
function fillForm(view) {
  ...
  view.element.querySelector('#cardNumber').value = '4000000000000002';
...
test('invalid card shows generic failure message', () => {
  ...
  controller.init();
  controller.show({ registrationId: 'reg_invalid', amount: 65, currency: 'USD' });
  ...
  const stored = paymentStorageService.getPaymentByRegistration('reg_invalid');
