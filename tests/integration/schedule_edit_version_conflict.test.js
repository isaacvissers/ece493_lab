...
test('reports version mismatch on edit', () => {
  ...
  scheduleRepository.saveDraft({
    conferenceId: conference.conferenceId,
    items: scheduleRepository.getScheduleItems(schedule.scheduleId),
  ...
  expect(view.element.textContent).toContain('Refresh');
  const logs = auditLogService.getLogs();
  expect(logs[0].eventType).toBe('schedule_concurrency');
});
