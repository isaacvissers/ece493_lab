...
function setValues(view) {
  view.element.querySelector('#conferenceId').value = 'conf_publish';
...
test('save and publish flow updates schedule status', () => {
  scheduleRepository.savePapers([
    { paperId: 'p1', status: 'accepted', requiredMetadataComplete: true, conferenceId: 'conf_publish' },
  ]);
  ...
  const schedule = scheduleRepository.getSchedule('conf_publish');
