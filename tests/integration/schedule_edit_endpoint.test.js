...
test('updates a schedule entry via the edit controller', () => {
  const conference = createConference({
    conferenceId: 'conf_integrated',
  ...
  view.element.querySelector('#editRoomId').value = 'room_c';
  ...
  expect(updated[0].roomId).toBe('room_c');
});
