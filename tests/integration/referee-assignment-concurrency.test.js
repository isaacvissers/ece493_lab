...
test('blocks assignment on concurrent change', () => {
  ...
  assignmentStorage.saveAssignments({
    paperId: 'paper_1',
    refereeEmails: ['x@example.com', 'y@example.com', 'z@example.com'],
    expectedVersion: 0,
  });
  setEmails(view, ['a@example.com', 'b@example.com', 'c@example.com']);
  submit(view);
  expect(view.element.querySelector('#assignment-banner').textContent).toContain('Assignment changed');
});
